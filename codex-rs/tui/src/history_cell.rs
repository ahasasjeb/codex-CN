...
impl HistoryCell for ReasoningSummaryCell {
    ...
    out.push("思考中".magenta().bold().into());
...
impl ExecCell {
                ...
                for p in call.parsed {
                    match p {
                        ParsedCommand::Read { name, .. } => {
                                lines.push(("读取", vec![name.into()]));
                        }
                        ParsedCommand::ListFiles { cmd, path } => {
                                lines.push(("列出", vec![path.unwrap_or(cmd).into()]));
                        }
    ...
    let title = if self.is_active() { "运行中" } else { "已运行" };
        let cmd_display = strip_bash_lc_and_escape(&call.command);
...
pub(crate) fn new_session_info(
    ...
    if is_first_event {
        ...
        let help_lines: Vec<Line<'static>> = vec![
            "  开始使用：描述一个任务或尝试以下命令：".dim().into(),
            Line::from(""),
            Line::from(vec![
                "  ".into(),
                "/init".into(),
                " - 创建一个 AGENTS.md 文件，用于向 Codex 提供说明".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/status".into(),
        ...
        let lines = vec![
            ...
            format!("请求: {}", config.model).into(),
            format!("使用: {model}").into(),
        ];
        CompositeHistoryCell {
...
impl HistoryCell for SessionHeaderHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        const CHANGE_MODEL_HINT_EXPLANATION: &str = " 来更改";
...
pub(crate) fn new_active_mcp_tool_call(invocation: McpInvocation) -> PlainHistoryCell {
    let title_line = Line::from(vec!["工具".magenta(), " 运行中...".dim()]);
...
pub(crate) fn new_completed_mcp_tool_call(
    ...
    match result {
        Ok(mcp_types::CallToolResult { content, .. }) => {
                ...
                for tool_call_result in content {
                    let line_text = match tool_call_result {
                        ...
                        mcp_types::ContentBlock::ImageContent(_) => {
                            // TODO 即使不是第一个结果也显示图像，需要重构 `CompletedMcpToolCall`
                            "<图像内容>".to_string()
                        }
            ...
            Err(e) => {
            lines.push(vec!["错误: ".red().bold(), e.into()].into());
        }
    };
...
pub(crate) fn new_limits_unavailable() -> PlainHistoryCell {
    PlainHistoryCell {
        lines: vec![
            "/limits".magenta().into(),
            "".into(),
            vec!["速率限制使用快照".bold()].into(),
            vec!["  提示: 在 Codex 回复后立即运行 `/limits` 以获取最新数据。".dim()].into(),
            vec!["  实时使用数据尚不可用。".into()].into(),
            vec!["  发送一条消息给 Codex，然后再次运行 /limits。".dim()].into(),
        ],
    }
}
...
pub(crate) fn new_status_output(
    ...
    // 📂 工作区
    lines.push(vec![padded_emoji("📂").into(), "工作区".bold()].into());
    ...
    lines.push(vec!["  • 路径: ".into(), cwd_str.into()].into());
    // Approval mode (as-is)
    lines.push(vec!["  • 审批模式: ".into(), lookup("approval").into()].into());
    ...
    lines.push(vec!["  • 沙盒: ".into(), sandbox_name.into()].into());
    ...
    if agents_list.is_empty() {
        lines.push("  • AGENTS 文件： (无)".into());
    } else {
        lines.push(vec!["  • AGENTS 文件： ".into(), agents_list.join(", ").into()].into());
    }
    lines.push("".into());
    ...
    {
    lines.push(vec![padded_emoji("👤").into(), "账户".bold()].into());
    lines.push("  • 已使用 ChatGPT 登录".into());
        ...
        match auth.openai_api_key.as_deref() {
            Some(key) if !key.is_empty() => {
                lines.push("  • 使用 API 密钥。运行 codex login 以使用 ChatGPT 计划".into());
            }
            _ => {
                let plan_text = info
    ...
    // 🧠 模型
    lines.push(vec![padded_emoji("🧠").into(), "模型".bold()].into());
    lines.push(vec!["  • 名称: ".into(), config.model.clone().into()].into());
    let provider_disp = pretty_provider_name(&config.model_provider_id);
    lines.push(vec!["  • 提供者: ".into(), provider_disp.into()].into());
    // Only show Reasoning fields if present in config summary
    let reff = lookup("reasoning effort");
    if !reff.is_empty() {
        lines.push(vec!["  • 推理强度: ".into(), title_case(&reff).into()].into());
    }
    let rsum = lookup("reasoning summaries");
    if !rsum.is_empty() {
        lines.push(vec!["  • 推理摘要: ".into(), title_case(&rsum).into()].into());
    }
    ...
    lines.push(vec![padded_emoji("💻").into(), "客户端".bold()].into());
    lines.push(vec!["  • CLI 版本: ".into(), cli_version.into()].into());
    lines.push("".into());

    // 📊 令牌使用情况
    lines.push(vec!["📊 ".into(), "令牌使用情况".bold()].into());
...
pub(crate) fn new_mcp_tools_output(
    ...
    lines.push(vec!["  • 服务器: ".into(), server.clone().into()].into());
...
impl HistoryCell for PlanUpdateCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let mut lines: Vec<Line<'static>> = vec![];
    lines.push(vec!["• ".into(), "已更新计划".bold()].into());
...
pub(crate) fn new_proposed_command(command: &[String]) -> PlainHistoryCell {
    ...
    let mut lines: Vec<Line<'static>> = Vec::new();
    lines.push(Line::from(vec!["• ".into(), "建议命令".bold()]));
...
pub(crate) fn new_reasoning_block(
    ...
    let mut lines: Vec<Line<'static>> = Vec::new();
    lines.push(Line::from("思考中".magenta().italic()));
...
mod tests {
    ...
    fn reasoning_summary_block() {
        ...
        assert_eq!(
            rendered_transcript,
            vec!["思考中", "Detailed reasoning goes here."]
        );
    }
    ...
    assert_eq!(rendered, vec!["思考中", "Detailed reasoning goes here."]);
    }
    ...
    fn reasoning_summary_block_falls_back_when_header_is_missing() {
        ...
        assert_eq!(
            rendered,
            vec!["思考中", "**High level reasoning without closing"]
        );
    }
    ...
    fn reasoning_summary_block_splits_header_and_summary_when_present() {
        ...
        assert_eq!(
            rendered_transcript,
            vec!["思考中", "We should fix the bug next."]
        );
    }
}
