    ...
    fn take_warnings(&mut self, weekly_used_percent: f64, hourly_used_percent: f64) -> Vec<String> {
            ...
            && weekly_used_percent >= RATE_LIMIT_WARNING_THRESHOLDS[self.weekly_index]
        {
            ...
            warnings.push(format!(
                "每周使用量已超过限制的 {threshold:.0}%。使用 /limits 查看详细用量。"
            ));
            self.weekly_index += 1;
        }
            ...
            && hourly_used_percent >= RATE_LIMIT_WARNING_THRESHOLDS[self.hourly_index]
        {
            ...
            warnings.push(format!(
                "每小时使用量已超过限制的 {threshold:.0}%。使用 /limits 查看详细用量。"
            ));
            self.hourly_index += 1;
        }
...
impl ChatWidget {
    ...
    let banner = format!(">> 代码审查开始: {} <<", review.user_facing_hint);
    ...
    pub(crate) fn open_model_popup(&mut self) {
        ...
        self.bottom_pane.show_selection_view(SelectionViewParams {
            title: "选择模型与推理等级".to_string(),
            subtitle: Some(
                "在此会话及后续 Codex CLI 会话中切换 OpenAI 模型".to_string(),
            ),
                ...
                move |tx: &AppEventSender| {
                    tx.send(AppEvent::CodexOp(Op::Review {
                        review_request: ReviewRequest {
                            prompt: "审查当前的代码变更（已暂存、未暂存和未跟踪的文件），并提供优先级排序的发现。".to_string(),
                            user_facing_hint: "当前更改".to_string(),
                        },
                    }));
                },
            )],
            ...
            items.push(SelectionItem {
                ...
                actions: vec![Box::new(move |tx3: &AppEventSender| {
                    tx3.send(AppEvent::CodexOp(Op::Review {
                        review_request: ReviewRequest {
                            prompt: format!(
                                "审查相对于基分支 '{branch}' 的代码更改。首先找到当前分支与 {branch} 之间的合并基点 (例如: git merge-base HEAD {branch})，然后对该 SHA 运行 `git diff` 以查看将合并到 {branch} 的更改。请提供有优先级的、可执行的发现。"
                            ),
        ...
        for entry in commits {
            ...
            items.push(SelectionItem {
                ...
                actions: vec![Box::new(move |tx3: &AppEventSender| {
                    ...
                    let prompt = format!(
                        "审查由提交 {sha} (\"{subject}\") 引入的代码更改。请提供有优先级的、可操作的发现。"
                    );
                    tx3.send(AppEvent::CodexOp(Op::Review {
                        review_request: ReviewRequest {
                            prompt,
        ...
        match self {
            Notification::AgentTurnComplete { response } => {
                Notification::agent_turn_preview(response)
                    .unwrap_or_else(|| "模型回应已完成".to_string())
            }
            Notification::ExecApprovalRequested { command } => {
                format!("请求批准: {}", truncate_text(command, 30))
            }
