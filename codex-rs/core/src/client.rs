    ...
    async fn stream_responses(&self, prompt: &Prompt) -> Result<ResponseStream> {
        ...
        let text = if self.config.model_family.family == "gpt-5" {
            ...
            if self.config.model_verbosity.is_some() {
                warn!(
                    "已设置 model_verbosity，但在非 gpt-5 模型家族中被忽略：{}",
        ...
        loop {
            ...
            match res {
                ...
                Ok(res) => {
                    ...
                    // OpenAI Responses 端点即使在返回 4xx/5xx 错误时也会返回结构化的 JSON 正文。
                    // 如果我们只上抛 HTTP 状态，调用方会看到一个不透明的
                    // "unexpected status 400 Bad Request"，这会让调试几乎不可能。
                    // 因此在这里读取（并包含）响应文本，让更高层和用户能看到精确的错误信息
                    //（例如："Unknown parameter: 'input[0].metadata'"）。响应体很小，并且该分支只在
                    // 错误路径上运行，因此额外的分配可忽略不计。
    ...
    S: Stream<Item = Result<Bytes>> + Unpin,
{
            ...
            Ok(None) => {
                match response_completed {
                    ...
                    None => {
                        ...
                        "在 response.completed 之前流已关闭".into(),
                                None,
                            ))))
                            .await;
                    }
    ...
    trace!("SSE 事件：{}", raw);
        ...
        let event: SseEvent = match serde_json::from_str(&sse.data) {
            Ok(event) => event,
            Err(e) => {
                debug!("解析 SSE 事件失败：{e}，数据：{}", &sse.data);
                continue;
            }
        };
        ...
        match event.kind.as_str() {
            ...
            "response.output_item.done" => {
                ...
                let Ok(item) = serde_json::from_value::<ResponseItem>(item_val) else {
                    debug!("无法从 output_item.done 解析 ResponseItem");
                    continue;
                };
            ...
            "response.failed" => {
                if let Some(resp_val) = event.response {
                    response_error = Some(CodexErr::Stream(
                        "收到 response.failed 事件".to_string(),
                        None,
                    ));
            ...
            "response.completed" => {
                if let Some(resp_val) = event.response {
                    match serde_json::from_value::<ResponseCompleted>(resp_val) {
                        ...
                        Err(e) => {
                            debug!("解析 ResponseCompleted 失败：{e}");
                            continue;
                        }
                    };
                };
